...
├── tokens.css        ← canonical CSS variables (colors + type scale)
...
The whole system lives in one CSS file: [`tokens.css`](./tokens.css). Import it and use the vars.
...
11 tokens. Quem usa: `tokens.css` (CSS vars), `ui_kits/web_app/styles.css` (mesma origem).
...
**Substituições / origem:** os três famílias são Google Fonts gratuitas e nenhum arquivo foi entregue. `tokens.css` carrega tudo via `@import` do CSS do Google Fonts. Se o produto migrar para `next/font` em produção (recomendado para zero CLS), copiar os mesmos pesos.
...
- **Fonts:** Unbounded / Inter / JetBrains Mono são todas Google Fonts gratuitas. Nenhum arquivo `.ttf` ou `.woff2` foi entregue — o `tokens.css` puxa do Google. Se você precisa hospedar localmente (offline, ou perf crítica), baixe os pesos especificados e referencie via `@font-face`.
...
Dark mode liga via `<html data-theme="dark">` (ou em qualquer ancestral). Os tokens em `tokens.css` se reescrevem automaticamente — sem necessidade de mudar componentes. Os ajustes vivem em um único bloco `[data-theme="dark"] { ... }` no CSS.
...
Texto preto sobre amarelo continua preto mesmo em dark. Isso é garantido por uma regra de _re-escopo_ em `tokens.css`:
